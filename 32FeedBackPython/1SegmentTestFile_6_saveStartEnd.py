...
parser.add_argument('--SegTestPred', dest='SegTestPred', default='SegmentTestPred/', help='directory of data')
...
def combineCsiLabel(saveDir,dataDir):
    ...
    #os.chdir(root_path)
    file_DataList=os.listdir(dataDir)
    for i_file in range(0,len(file_DataList)):
        i = file_DataList[i_file]
        #print(i[11:16])
        if(i[11:16] == 'label'):
            file_DataList[i_file] = ''
    while '' in file_DataList:
        file_DataList.remove('')

    file_LbelList = os.listdir(dataDir)
    for i_file in range(0,len(file_LbelList)):
        i = file_LbelList[i_file]
        #print(i[11:15])
        if(i[11:15] == 'data'):
            file_LbelList[i_file] = ''
    while '' in file_LbelList:
        file_LbelList.remove('')
...
def main(_):
    ...
    #SIZE_LABEL = []
    #predResultDir = 'SegmentTestPred/'
    predResultDir = args.SegTestPred + '/'
    segmentResultDir='SegmentResultPerFile/'
    saveDir='CombineDir/'    
    ...
    #allFileName = ['user1_iw_6.mat','user1_ph_6.mat','user1_rp_6.mat','user1_sd_6.mat','user1_wd_6.mat']
    allFileName = []
    ...
    for outFileName in allFileName :
        ...
        data_dir = 'OriginalMatData/'
        outFileName = outFileName + '.mat'
        originalFile= data_dir + '55' + outFileName
        size_labelOne = actionSampleExtract(startEndPoints,originalFile,outFileName,segmentResultDir)
        saveStartEndPoints(saveDir,outFileName,startEndPoints) # save start and end points in .csv file
        
    combineStartEndPoints(saveDir)
    #saveDir='CombineDir/'
...
def saveStartEndPoints(saveDir,outFileName,startEndPoints):
    fStartEnd = open(saveDir+ '/'+ outFileName.replace('.mat','.csv'),'w')
    len_A=len(startEndPoints) #10个动作
    for i in range(0,len_A):
        fStartEnd.write(str(i+1) + ',' + str(int(startEndPoints[i,0])) + ',' + str(int(startEndPoints[i,1])) + '\n')
    fStartEnd.close()
def combineStartEndPoints(saveDir):
    fUser1 = open('user1ManualSegment.csv','w')
    fUser2 = open('user2ManualSegment.csv','w')
    fUser3 = open('user3ManualSegment.csv','w')
    fUser4 = open('user4ManualSegment.csv','w')
    fUser5 = open('user5ManualSegment.csv','w')
    fopenAll = [fUser1,fUser2,fUser3,fUser4,fUser5]
    allFiles = os.listdir(saveDir)
    kk = 0
    for oneFile in allFiles:
        if(oneFile.split('.')[-1] == 'csv'):
            print(oneFile)
            fStartEnd = open(saveDir+ '/'+ oneFile)
